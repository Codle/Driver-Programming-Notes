...
1. 编写一个字符驱动程序，实现以下功能：
   + 有统一的读写缓冲区，所有的读写针对该缓冲区进行
   + 缓冲区默认的大小为1000BYTE，可以通过模块参数修改
   + 实现ioctl，包括设置缓冲区的大小，缓冲区中已有的数据个数（字节数），修改读写指针位置，清除缓冲区已有的内容（修改文件指针）
   + 实现对该设备的数据的READ,WRITE,LSEEK功能
   + 当设备缓冲区满或空的情况下，将读写进程置成阻塞（未实现）
   + 对临界资源则进行互斥访问（未实现）
2. 将驱动程序加载到系统内核中，并在/dev下建立相应的节点文件
3. 编写应用程序对驱动程序进行测试
